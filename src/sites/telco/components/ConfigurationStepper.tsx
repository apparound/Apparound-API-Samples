        ...
        <div key={step.label} className="flex items-center">
          <div className="flex flex-col items-center">
            <div className={`flex items-center justify-center w-6 h-6 rounded-full mb-1
              ${step.completed ? "bg-green-500" : step.current ? "border-2 border-purple-600" : "border-2 border-gray-300"}`}>
              {step.completed ? (
                <Check className="w-4 h-4 text-white" />
              ) : (
                <Circle className={`w-3 h-3 ${step.current ? "text-purple-600" : "text-gray-300"}`} />
              )}
            </div>
            <span className={`text-xs ${step.current ? "text-purple-600 font-medium" : "text-gray-500"}`}>
              {step.label}
            </span>
          </div>
