...
interface FormProps {
    ...
    className?: string,
    onChange: (value: string|number|boolean, name: string, item: any) => any
}
            ...
            {children.map((item, index) => {
                ...
                return InputComponent ? (
                    <div className={`${sizes[item.size]}`} key={index}>
                        <InputComponent values={values} {...item} onChange={(value, name, currentItem) => onChange(value, name, currentItem || item)} />
                    </div>
                ) : null
            })}
        </div>
