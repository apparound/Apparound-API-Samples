...
const flatCart = (cart: Record<string, any>): string[] => {
    ...
    return [
        ...Object.keys(cart),
        ...Object.values(cart).flatMap(flatCart)
    ]
}
...
export const quoteSlice = createSlice({
    ...
    reducers: {
        ...
        addProduct: (state, { payload }) => {
            const { productGuid, parentGuid, products, quote, solutionId, validation } = payload
...
export const selectCartClusters = createSelector([
    ...
    return {
        mainCluster,
        selectedSize,
        clusters: selectedSize?.clusters?.map(cluster => ({
            ...cluster,
...
export const { reset, initQuote, addProduct, deleteProduct, addToCart, updateContractProperties, updateCustomer, updateQuote, updateContract } = quoteSlice.actions
