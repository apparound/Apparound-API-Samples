...
import {
    ...
    updateQuote,
    updateContract
} from '@/sites/retail/features/quoteSlize'
...
export const addProduct = async (productGuid: string, dispatch: any, parentGuid?: string, getParentProducts?: boolean) => {
    const response = await fetchData(`/addProduct/productGuid/${productGuid}/parentGuid/${parentGuid || ''}`, 'get')

    const products = await fetchData(`/getProducts/productGuid/${getParentProducts && parentGuid ? parentGuid : productGuid}`, 'get')
...
export const saveContract = async (contract, customer, dispatch) => {
    await fetchData('/updateCustomerQuote', 'post', { customer: {
        ...customer,
        properties: {
            ...customer
        }
