...
const ProductItem = ({ product, withSeparator, priceModel }: productItemI) =>
{
    ...
    return (
        <div className={product.label.startsWith('F') ? 'pl-4' : null}>
            ...
            <Content title={product.label} info={product.price ? `${(product.price / product.quantity).toFixed(2)}€/${priceModel || ''}` : null} />
            {product.children.map((innerProduct, index) =>
            {
                return (
                    <ProductItem product={innerProduct} withSeparator={true} priceModel={priceModel} key={index} />
                )
            })}
        </div>
    )
